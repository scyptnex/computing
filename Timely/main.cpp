...
// The only valid use of a macro is for token-pasting, as shown below
#define describeTime(t) std::cout << #t << ": " << (t).getTimeAsString() << std::endl
#define describeComparison(tl,op,tr) std::cout << #tl << " " << #op << " " << #tr << " -> " << (tl op tr ? "true" : "false") << std::endl

using namespace week06;
...
void example()
{
    ...
    //make sure its methods work
    std::cout << ti.numberOfHours() << ", " << ti.numberOfMinutes() << ", " << ti.numberOfSeconds() << std::endl;
    std::cout << std::endl;
    ...
    Time tAssign;
    tAssign = ti;
    describeTime(tAssign);

    Time tAdd = ti + Time(1,1,1);
    describeTime(tAdd);

    Time tSub = ti - Time(1,1,1);
    describeTime(tSub);

    describeComparison(ti,==,ti);
    describeComparison(ti,==,tAdd);
    describeComparison(ti,!=,ti);
    describeComparison(ti,!=,tAdd);
    describeComparison(ti,<,ti);
    describeComparison(ti,<,tAdd);
    ...
    Time tSpecialAdd = ti + 100;
    describeTime(tSpecialAdd);
}
